...
sys.path.append("..")
from image_utils import DataProvider
...
def train_model(model,trainprovider,testprovider,model_dir,**kwargs):
    ...
    train_folder_name = datetime.strftime(datetime.now(),"%m%d-%H:%M:%S")
    ...
    with tf.Session(graph=model.graph) as sess:
        ...
        optm = tf.train.AdamOptimizer(learning_rate=0.0001).minimize(model.cost) # optimizer
        ...
        for index in range(1,n_epochs+1):
            ...
            for _ in range(train_nums // batch_size):
                ...
                sess.run(optm, feed_dict={"input:0":x,"label:0":y,"keep_prod:0":keep_prod,})
            ...
            for idx in range(test_nums):
                ...
                pred_mask = cv2.cvtColor(pred[0,...,1].astype(np.float32),cv2.COLOR_GRAY2RGB)
                arg_pred_mask = cv2.cvtColor(arg_pred[0,...].astype(np.uint8),cv2.COLOR_GRAY2RGB)
                case = np.concatenate([img,pad,ground_truth,pad,pred_mask,pad,arg_pred_mask],axis=1)
                case_list.append(case)
            testLoss = np.mean(testLoss)
            testAcc = np.mean(testAcc)
            #### ---- MODEL SAVE ---- ####
            model_name = "{:03d}_{:2.2f}.ckpt".format(index,testAcc*100)
...
if __name__ == "__main__":
    ...
    "channels":3,
    "n_class":2, # output 갯수
    "layers":4, # layer depth
    "feature_root":64, # 첫 features root 깊이가 깊어짐에 따라 2배
    "kernel_size":3,
    "pool_size":2,
    "l2_scale":None # l2_regularizer
    }
    with open(os.path.join(model_dir,"model_config.yml"), 'w') as yaml_file:
        yaml.dump(unet_config, yaml_file, default_flow_style=False)

    model = Unet(**unet_config)
    trainprovider = DataProvider("../data/train/image/","../data/train/disk/")
    testprovider = DataProvider("../data/test/image/","../data/test/disk/")
